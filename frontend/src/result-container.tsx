...
export default function ResultContainer(props: ResultContainerProps): JSX.Element {
    ...
    return <Container style={{paddingBottom: '3em'}}>
        ...
        <Typography variant='h6' style={{paddingBottom: '1em'}}>Safety score:</Typography>
        <div style={{display: 'flex', flexFlow: 'row', justifyContent: 'center'}}>
        <div style={{width: '70%', display: 'flex', justifyContent: 'center', alignItems: 'center'}}>
            <LinearProgress variant="determinate" value={safetyScore} color={isSafe ? 'success' : 'error'} style={{width: '30%'}}/>
            <div style={{paddingLeft: '1em'}}><Typography
          variant="body2"
          color={isSafe ? 'success' : 'error'}
        >{`${Math.round(safetyScore)}%`}</Typography></div>
            </div>
        </div>
        </Container>
}